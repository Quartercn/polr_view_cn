$(function() {
    ...
    $('#check-link-availability').click(function() {
        ...
        $('#link-availability-status').html('<span><i class="fa fa-spinner"></i> 加载中</span>');
        request.done(function(msg) {
            if (msg == 'unavailable') {
                $('#link-availability-status').html(' <span style="color:red"><i class="fa fa-ban"></i> 已经被使用了</span>');
            } else if (msg == 'available') {
                $('#link-availability-status').html('<span style="color:green"><i class="fa fa-check"></i> 可使用</span>');
            } else if (msg == 'invalid') {
                $('#link-availability-status').html('<span style="color:orange"><i class="fa fa-exclamation-triangle"></i> 不合法的自定义 URL 后缀</span>');
            } else {
    ...
    function changeTips(tcase) {
        switch (tcase) {
            case 1:
                setTip('创建一个帐户以跟踪您的链接');
                break;
            case 2:
                setTip('您是否知道可以通过单击“链接选项”来更改 URL 的后缀？');
                i = 1;
                break;
        }
    }
