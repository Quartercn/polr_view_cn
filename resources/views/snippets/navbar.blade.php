    ...
    <nav role="navigation" class="navbar navbar-default navbar-fixed-top">
        ...
        <ul id="navbar" class="nav navbar-collapse collapse navbar-nav" id="nbc">
            ...
            @if (empty(session('username')))
                ...
                @endif
            @else
                <li class="visible-xs"><a href="{{ route('admin') }}">面板</a></li>
                <li class="visible-xs"><a href="{{ route('admin') }}#settings">设置</a></li>
                <li class="visible-xs"><a href="{{ route('logout') }}">注销</a></li>
            @endif
        </ul>
                ...
                <li class="dropdown">
                    ...
                    <div class="dropdown-menu pull-right login-dropdown-menu" id="dropdown">
                        ...
                        <form action="login" method="POST" accept-charset="UTF-8">
                            ...
                            <input type="password" name="password" placeholder='密码' size="30" class="form-control login-form-field" />
                            <input type="hidden" name='_token' value='{{csrf_token()}}' />
                            <input class="btn btn-success form-control login-form-submit" type="submit" name="login" value="登录" />
                        </form>
                    </div>
                </li>
            @else
                <div class='nav pull-right navbar-nav'>
                    ...
                    <a class="dropdown-toggle login-name" href="#" data-toggle="dropdown">{{session('username')}} <strong class="caret"></strong></a>
                        <ul class="dropdown-menu pull-right" role="menu" aria-labelledby="dropdownMenu">
                            <li><a tabindex="-1" href="{{ route('admin') }}">面板</a></li>
                            <li><a tabindex="-1" href="{{ route('admin') }}#settings">设置</a></li>
                            <li><a tabindex="-1" href="{{ route('logout') }}">注销</a></li>
                        </ul>
                    </li>
                </div>
            @endif
